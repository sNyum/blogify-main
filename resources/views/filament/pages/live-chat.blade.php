<x-filament-panels::page>
    <div x-data="adminChat()" x-init="init()" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="space-y-2 max-h-[600px] overflow-y-auto">
                <template x-for="conversation in conversations" :key="conversation.id">
                    <div 
                        ...
                        :class="selectedConversationId == conversation.id ? 'bg-primary-100 dark:bg-primary-900' : 'bg-gray-50 dark:bg-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600'"
                        class="p-3 rounded-lg cursor-pointer transition-colors"
                    >
                        <div class="flex items-start justify-between">
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-medium text-gray-900 dark:text-white truncate" x-text="conversation.external_user?.name || 'Unknown User'"></p>
                        ...
                        <div class="flex items-center justify-between">
                            <div>
                                ...
                                <p class="text-sm text-gray-500 dark:text-gray-400">
                                    <span x-text="currentConversation.external_user?.name"></span>
                                    (<span x-text="currentConversation.external_user?.email"></span>)
                                </p>
                            </div>
                            <div class="flex gap-2">
                                <template x-if="currentConversation.status === 'open'">
                                    <button 
                                        @click="assignToMe()"
                            ...
                            <div :class="message.sender_type === 'admin' ? 'flex justify-end' : 'flex justify-start'">
                                <div class="max-w-[70%]">
                                    <div class="text-xs text-gray-500 dark:text-gray-400 mb-1">
                                        <span x-text="message.sender_type === 'admin' ? (message.sender?.name || 'Admin') : (message.sender_type === 'system' ? 'System' : currentConversation.external_user?.name)"></span>
                                        <span class="ml-1" x-text="new Date(message.created_at).toLocaleTimeString('id-ID', {hour: '2-digit', minute: '2-digit'})"></span>
                                    </div>
                                    <div 
                                        :class="{
                                            'bg-primary-600 text-white': message.sender_type === 'admin',
        ...
        function adminChat() {
            return {
                ...
                async loadConversations() {
                    try {
                        const response = await fetch(`/api/admin/chat/conversations?status=${this.statusFilter}`, {
                            headers: {
                ...
                async loadMessages() {
                    ...
                    try {
                        const response = await fetch(`/api/admin/chat/conversations/${this.selectedConversationId}/messages`, {
                            headers: {
                                'Authorization': `Bearer ${document.querySelector('meta[name="csrf-token"]').content}`,
                                'Accept': 'application/json'
                            }
                ...
                async assignToMe() {
                    ...
                    try {
                        await fetch(`/api/admin/chat/conversations/${this.selectedConversationId}/assign`, {
                            method: 'POST',
                            headers: {
                                'Authorization': `Bearer ${document.querySelector('meta[name="csrf-token"]').content}`,
                                'Accept': 'application/json'
                ...
                async closeConversation() {
                    ...
                    try {
                        await fetch(`/api/admin/chat/conversations/${this.selectedConversationId}/close`, {
                            method: 'POST',
                            headers: {
                                'Authorization': `Bearer ${document.querySelector('meta[name="csrf-token"]').content}`,
                                'Accept': 'application/json'
